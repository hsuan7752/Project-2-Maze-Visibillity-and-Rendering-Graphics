...
const float zNear = 0.01, zFar = 200;
...
//======================================================================
{
	...
	{
		float edge_start[2] = { this->edges[i]->endpoints[Edge::START]->posn[Vertex::X],
								this->edges[i]->endpoints[Edge::START]->posn[Vertex::Y] };
		float edge_end[2] = { this->edges[i]->endpoints[Edge::END]->posn[Vertex::X],
	...
	/*float edge_start[2] = { this->edges[0]->endpoints[Edge::START]->posn[Vertex::X],
								this->edges[0]->endpoints[Edge::START]->posn[Vertex::Y] };
	float edge_end[2] = { this->edges[0]->endpoints[Edge::END]->posn[Vertex::X],
						  this->edges[0]->endpoints[Edge::END]->posn[Vertex::Y] };

	float color[3] = { this->edges[0]->color[0], this->edges[0]->color[1], this->edges[0]->color[2] };
	if (this->edges[0]->opaque)
		Draw_Wall(edge_start, edge_end, color);*/
...
{
	float edge0[3] = {start[Y], 1.0f, start[X]};
	float edge1[3] = { end[Y], 1.0f, end[X] };
	float edge2[3] = { end[Y], -1.0f, end[X] };
	float edge3[3] = { start[Y], -1.0f, start[X] };
...
//		of 4 * 4 matrix
//   b	Point to 4 consecutive values that are used as the elements 
//		of 4 * 1 matrix
//======================================================================
float* Maze::
Multiplication(float* a, float* b)
{
	float* Vertex = new float[4]{ 0 };

	for (int i = 0; i < 4; ++i)
		for (int j = 0; j < 4; ++j)
			Vertex[i] += a[j * 4 + i] * b[j];

	return Vertex;
}

float* Maze::
Normalize(float* vertex)
{
	vertex[0] /= (float)abs(vertex[3]);
	vertex[1] /= (float)abs(vertex[3]);
	vertex[2] /= (float)abs(vertex[3]);
